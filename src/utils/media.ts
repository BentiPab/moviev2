...
export const getImagePath = (url?: string, original?: boolean, casting?: boolean) => {
    if (!url) {
...
const mapProviders = (provider: WatchProvider[]) =>
...
export const formatProviders = (providers: WatchProviders) => {
    const buyProviders = providers.buy && mapProviders(providers.buy);
    const streamProviders =
        providers.flatrate && mapProviders(providers.flatrate);
    const rentProviders = providers.rent && mapProviders(providers.rent);
